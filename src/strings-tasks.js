...
function concatenateStrings(strOne, strTwo) {
  const wholeStr = strOne.concat(strTwo);
  return wholeStr;
}
...
function removeLastOccurrences(str, subStr) {
  ...
  if (lastIndex !== -1) {
    return (
      str.substring(0, lastIndex) + str.substring(lastIndex + subStr.length)
    );
  }
  return str;
}
...
function unbracketTag(str) {
  if (str.length < 2) {
    throw new Error('Invalid input: Tag must have at least two characters.');
  }
